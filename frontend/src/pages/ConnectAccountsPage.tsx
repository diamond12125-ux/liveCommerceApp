...
const ConnectAccountsPage = () => {
  ...
  const connectFacebook = () => {
    ...
    if (!clientId) {
      toast.error('Facebook App ID not configured. Please set VITE_FACEBOOK_APP_ID in .env file');
      return;
    }
  ...
  const connectYouTube = () => {
    // Support both VITE_YOUTUBE_CLIENT_ID and VITE_GOOGLE_CLIENT_ID
    const clientId = import.meta.env.VITE_YOUTUBE_CLIENT_ID || import.meta.env.VITE_GOOGLE_CLIENT_ID;

    if (!clientId) {
      toast.error('YouTube OAuth not configured. Please VITE_YOUTUBE_CLIENT_ID in .env file');
      return;
    }

    const redirectUri = `${window.location.origin}/oauth/youtube/callback`;
    const scope = import.meta.env.VITE_YOUTUBE_OAUTH_SCOPE || 'https://www.googleapis.com/auth/youtube https://www.googleapis.com/auth/youtube.force-ssl';
  ...
  const connectInstagram = () => {
    ...
    if (!clientId) {
      toast.error('Instagram Client ID not configured. Please set VITE_INSTAGRAM_CLIENT_ID in .env file');
      return;
    }
